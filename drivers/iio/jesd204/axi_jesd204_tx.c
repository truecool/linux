	...
	const struct jesd204_tx_config *config)
{
    ...
    chksum += config->lanes_per_device;
    chksum += config->enable_scrambling;
    chksum += config->octets_per_frame;
    chksum += config->frames_per_multiframe;
    chksum += config->converters_per_device;
    chksum += config->resolution;
    chksum += config->bits_per_sample;
    chksum += config->subclass_version;
    chksum += config->samples_per_frame;
	...
	for (lane = 0; lane < jesd->num_lanes; lane++) {
		...
		for (i = 0; i < 4; i++) {
			...
			case 1:
				...
				break;
			case 2:
				val = config->converters_per_device;
	...
	struct axi_jesd204_tx *jesd, struct jesd204_tx_config *config)
{
	...
	config->converters_per_device = 2;
